...
def parse_usalign_file(file_object,alignment_type):
    ...
    :return: dictionary of quantitative results associated with the alignment. 
             Keys:
                'struct1': string, path to the query structure; always assumed 
                           to be the model structure
                'struct2': string, path to the target structure; always assumed 
                           to be the target structure
                ...
                'rmsd': float, root mean square deviation between the aligned 
                        atoms
                'alnSeqID': float, sequence identity of aligned residues
                'Len1': float, the query structure's length
                'Len2': float, the target structure's length
                'alnLen': float, the number of residues used in the alignment
                ...
                'd0_2': float, the normalized distance for target structure; used 
                        in TMscore metric calculation
                'map_1_to_2': dictionary, keys are query structure's ('1') 
                              residue indices (string) that map to a tuple with 
                              query residue name, target structure's ('2') 
                              residue index (string), target residue name, and 
                              alignment distance (float, only available for SNS 
                              and FNS alignments); if alignment_type is not in 
                              ['CP', 'SNS', 'FNS'], this mapping will not be 
                              created
    """
...
def threshold_comparison(results_dict,ranking_metric_string,metric_cutoff):
    ...
    """
    if ranking_metric_string.upper() not in ['TMSCORE1','TMSCORE2','MAXTMSCORE','AVGTMSCORE']:
